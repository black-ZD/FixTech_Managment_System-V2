...
exports.getSummary     = async(req,res)=>{ try{res.json({success:true,data:await svc.getSummary()});}catch(e){res.status(500).json({success:false,message:e.message});} };
exports.getMonthlyTrend= async(req,res)=>{ try{res.json({success:true,data:await svc.getMonthlyTrend()});}catch(e){res.status(500).json({success:false,message:e.message});} };
